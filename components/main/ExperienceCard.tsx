...
export default function ExperienceCard({ experience }: { experience: Experience[] }) {
  return (
    <div className="p-2 text-[#dfe4ed]">
      ...
      {experience.length === 0 ? (
        <p className="text-[#dfe4ed]/60">No work experience yet.</p>
      ) : (
        <div className="relative border-l border-[#dfe4ed]/30 pl-6">
          {experience.map((exp, index) => (
            <div key={index} className="mb-8 relative group">
              ...
              >
                <h3 className="font-semibold text-[#dfe4ed] text-lg">
                  {exp.role} <span className="text-[#dfe4ed]/70">@ {exp.company}</span>
                </h3>
